...
const ForeCastForm = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            const positionValues = response.data.results[0].position_values;
            state.one_three_average = response.data.results[0].one_three_average;
            state.four_ten_average = response.data.results[0].four_ten_average;
    ...
    const createForecast = (event: React.FormEvent) => {
        ...
        try {
            axios.post(baseConst.apiUrl + 'v1/forecast/create/', postData, {
                headers: {
                  Authorization: `Token ${token}`,
                  Accept: 'application/json',
                  'Content-Type': 'application/json',
    ...
    <Card>
        ...
        <form onSubmit={e => e.preventDefault()}>
            ...
            <div>
                ...
                {Boolean(errors.fields) && <FormHelperText style={{ color: '#FF4C51' }}>{errors.fields}</FormHelperText>}
                
            </div>
            </Grid>
            <Divider sx={{ mt: 0, mb: 1 }} style={{borderBottom: '1px solid #ccc', width: '100%', marginTop: '20px'}} />
            <Grid item xs={12}>
                ...
                <FormControl fullWidth>
                    <Autocomplete
                        ...
                        filterOptions={(options, { inputValue }) =>
                            options.filter(
                            (option) =>
                                option.language_name.toLowerCase().includes(inputValue.toLowerCase())
                            )
                        }
                    />
                    {Boolean(errors.selectedLanguage) && <FormHelperText style={{ color: '#FF4C51' }}>{errors.selectedLanguage}</FormHelperText>}
                </FormControl>
                </Box>
            </Grid>
            <Grid item xs={3}>
                ...
                <FormControl fullWidth>
                    ...
                    {Boolean(errors.device) && <FormHelperText style={{ color: '#FF4C51' }}>{errors.device}</FormHelperText>}
                </FormControl>
                </Box>
            </Grid>
            <Grid item xs={3}>
            ...
            <Grid item xs={12}>
                ...
                />
                {Boolean(errors.selectedLocation) && <FormHelperText style={{ color: '#FF4C51' }}>{errors.selectedLocation}</FormHelperText>}
                </FormControl>
                </Box>
            </Grid>
